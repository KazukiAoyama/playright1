...
### 1.3 共通仕様
* **定期実行 (GitHub Actions)**: さいたま市公共施設予約システムの定期自動スクレイピング（Ubuntu環境、cron実行）を行い `docs/data_city.json` をコミット・更新（※埼玉県営公園APIはクラウド海外IP制限があるためGitHub Actionsからは除外）。
* **オンタイム即時取得 (埼玉県営・秋ヶ瀬公園)**: Direct API により約2秒で取得可能なため、ローカルWebサーバー（`server.py`）経由でWebページロード時にオンタイム（即時）で実行・取得。
* **データ公開**: `docs/index.html` 上で両データをシームレスに統合表示（**日本の祝日・土日限定フィルタ、午前・午後別表示、明るい高密度レイアウト対応**）。
