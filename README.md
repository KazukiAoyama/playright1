...
### 1.3 共通仕様
* **定期実行 (GitHub Actions)**: さいたま市公共施設予約システムの定期自動スクレイピング（Ubuntu環境、cron実行）を行い `docs/data_city.json` をコミット・更新。
* **オンタイム実行 (埼玉県営・秋ヶ瀬公園)**: Direct API により約2秒で取得可能なため、ローカルWebサーバー（`server.py`）のAPIを介してWebページのロード時にリアルタイム（オンタイム）で実行・取得。
