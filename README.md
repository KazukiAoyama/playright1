...
### 1.3 共通仕様
* **定期実行 (GitHub Actions)**: さいたま市および埼玉県営（秋ヶ瀬公園）の定期自動スクレイピング（Ubuntu環境、cron実行）を行い、`docs/data.json` および `docs/data_city.json` をコミット・更新。
* **ローカル即時取得 (`server.py`)**: ローカルWebサーバー経由でアクセスした際は、Direct APIによりWebページロード時に秋ヶ瀬公園の最新枠をオンタイム（即時）で取得・表示。
* **データ公開**: `docs/index.html`（GitHub Pages）上で両データをシームレスに統合表示（**日本の祝日・土日限定フィルタ、午前・午後別表示、明るい高密度レイアウト対応**）。
